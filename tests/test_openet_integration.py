...
def test_resolve_monthly_et_in_falls_back_on_request_timeout(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("OPENET_API_KEY", "test-key")

    with patch("helios.utils.openet.requests.post", side_effect=requests.Timeout("slow OpenET response")):
        value, source = resolve_monthly_et_in(
            longitude=-114.45,
            latitude=43.60,
            observed_at=datetime(2024, 7, 5, tzinfo=timezone.utc),
        )
